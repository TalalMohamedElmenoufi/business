...
$pergunta = "select * from usuarios where token != '' ";
...
while($d = mysqli_fetch_object($resultado)){
	...
	if($celularDb[1]=='92' or $celularDb[1]=='85' or $celularDb[1]=='82'){
		...
		$cellsCliente = $celularDb[0].''.$celularDb[1].''.$tira;
	}else{
		$cellsCliente = $celularDb[0].''.$celularDb[1].''.$tira;
		//$cellsCliente = $celularDb[0].''.$celularDb[1].''.$celularDb[2].''.$tira;//caso acrecente o 9
	}
	
	
$AnoMEs = date("Y-m");		
$pergunta2 = "select id_rg, id, quantidade_plano, description, value, status, customer from elmenoufi_bot_".$d->id.".asaas_cobranca_email 
where
...
ListaConexao($conexao,$conexao2,$timezone,$api_token,$token,$celular,$celulares,$ip,$porta,$d->id,$d2->id_rg,$d2->id,$d2->quantidade_plano,$d2->description,$d2->value,$d2->status,$d2->nome,$emails,$cellsCliente);	
...
function ListaConexao($conexao,$conexao2,$timezone,$api_token,$token,$celular,$celulares,$ip,$porta,$idCli,$id_rg,$pay_id,$quantidade_plano,$description,$value,$statusDb,$nome,$emails,$cellsCliente){
	
	list($nome,$credito_email) = mysqli_fetch_row(mysqli_query($conexao, "select nome,credito_email from usuarios where id = '".$idCli."' "));
	...
	StatusFatura($conexao,$conexao2,$timezone,$api_token,$token,$ip,$porta,$Clientes,$description,$value,$cells,$celulares,$idCli,$id_rg,$pay_id,$statusDb,$credito_email,$quantidade_plano,$nome,$emails,$cellsCliente);	
	
	
}

function StatusFatura($conexao,$conexao2,$timezone,$api_token,$token,$ip,$porta,$clienteDb,$description,$value,$cells,$celulares,$idCli,$id_rg,$pay_id,$statusDb,$credito_email,$quantidade_plano,$nome,$emails,$cellsCliente){
	...
	//echo $credito_email."<br>";
	//echo $quantidade_plano."<br>";
	
	$CreditarEmail = $credito_email + $quantidade_plano;
	//echo $CreditarEmail."<br>";
	...
	if($statusDb!=$statusApi){
		...
		if($statusApi){
			mysqli_query($conexao2," update asaas_cobranca_email set  dueDate='".$dueDate."',  value='".$valueApi."', netValue='".$netValueApi."', originalValue='".$originalValue."', interestValue='".$interestValue."', paymentDate='".$paymentDate."', clientPaymentDate='".$clientPaymentDate."', lastInvoiceViewedDate='".$lastInvoiceViewedDate."', lastBankSlipViewedDate='".$lastBankSlipViewedDate."', status='".$statusApi."' where id='".$pay_id."' ");		

			if($statusApi=='RECEIVED' or $statusApi=='CONFIRMED'){

				mysqli_query($conexao," update usuarios set credito_email = '".$CreditarEmail."' where id='".$idCli."' ");

				SendAlertas($conexao2,$ip,$porta,$token,$nome,$emailsDb,$clienteDb,$descriptionApi,$quantidade_plano,$statusApi,$cellsCliente); //acao para o cliente
			}
			 NotificarWhats($ip,$porta,$token,$cells,$celulares,$descriptionApi,$valueApi,$quantidade_plano,$clienteDb,$statusApi); //ação para TME
...
function SendAlertas($conexao2,$ip,$porta,$token,$nome,$emailsDb,$clienteDb,$descriptionApi,$quantidade_plano,$statusDb,$cellsCliente){
	...
	$Mensagem = "Olá $nome sua solicitação de $descriptionApi, quantidade: $quantidade_plano, com status: $statusApi ";
	 
	mysqli_query($conexao2," insert into alertas set
	alerta = '".($Mensagem)."',
	...
	$numeros[] = $cellsCliente;

	$mensagemEnviar = "Olá *$nome* sua solicitação de $descriptionApi, quantidade: *$quantidade_plano*, com status: *$statusApi* ";
