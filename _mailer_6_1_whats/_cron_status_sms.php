...
$AnoMEs = date("Y-m");		
$pergunta2 = "select id_rg, id, quantidade_plano, description, value, status from elmenoufi_bot_".$d->id.".asaas_cobranca_sms 
where
...
function ListaConexao($conexao,$conexao2,$timezone,$api_token,$token,$celular,$celulares,$ip,$porta,$idCli,$id_rg,$pay_id,$quantidade_plano,$description,$value,$statusDb,$nome,$emails){
	...
	list($nome,$creditos_sms) = mysqli_fetch_row(mysqli_query($conexao, "select nome,creditos_sms from usuarios where id = '".$idCli."' "));
	...
	StatusFatura($conexao,$conexao2,$timezone,$api_token,$token,$ip,$porta,$Clientes,$description,$value,$cells,$celulares,$idCli,$id_rg,$pay_id,$statusDb,$creditos_sms,$quantidade_plano,$nome,$emails);	
	
	
}

function StatusFatura($conexao,$conexao2,$timezone,$api_token,$token,$ip,$porta,$clienteDb,$description,$value,$cells,$celulares,$idCli,$id_rg,$pay_id,$statusDb,$creditos_sms,$quantidade_plano,$nome,$emails){
	...
	//echo $creditos_sms."<br>";
	//echo $quantidade_plano."<br>";
	
	$CreditarSMS = (($creditos_sms) + ($quantidade_plano));
	//echo $CreditarSMS."<br>";
	...
	if($statusDb!=$statusApi){
		...
		if($statusApi){
			mysqli_query($conexao2," update asaas_cobranca_sms set  dueDate='".$dueDate."',  value='".$valueApi."', netValue='".$netValueApi."', originalValue='".$originalValue."', interestValue='".$interestValue."', paymentDate='".$paymentDate."', clientPaymentDate='".$clientPaymentDate."', lastInvoiceViewedDate='".$lastInvoiceViewedDate."', lastBankSlipViewedDate='".$lastBankSlipViewedDate."', status='".$statusApi."' where id='".$pay_id."' ");		

			if($statusApi=='RECEIVED'){

				mysqli_query($conexao," update usuarios set creditos_sms = '".$CreditarSMS."', alert_sms = '0'  where id='".$idCli."'");
