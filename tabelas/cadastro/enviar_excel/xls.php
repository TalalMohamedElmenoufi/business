...
$data->read($caminho . $nomeAleatorio);
	...
	for ($i = 0; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				$nome = ((!empty($data->sheets[0]['cells'][$i][1])) ? $data->sheets[0]['cells'][$i][1] : false);
				$celular = ((!empty($data->sheets[0]['cells'][$i][2])) ? $data->sheets[0]['cells'][$i][2] : false);
				$email = ((!empty($data->sheets[0]['cells'][$i][3])) ? $data->sheets[0]['cells'][$i][3] : false);
		
				$data_nascimento = ((!empty($data->sheets[0]['cells'][$i][4])) ? $data->sheets[0]['cells'][$i][4] : false);
	
				$Da = $data_nascimento;
					
				$TD = explode('/',$data_nascimento) ;
				$dataTratada = $TD[2].'-'.$TD[1].'-'.$TD[0];		
		
	   			if ($Da){
					$dataT = date('Y-m-d',strtotime("-1 day", strtotime($dataTratada)));
				}
			...
			list($telBase) = mysqli_fetch_row(mysqli_query($conexao2, "select destination from cadastro where categoria = '".$_SESSION[$Script][CodCat]."' and destination='".$cells."'  "));			
		
	   		if($telBase!=$cells and $cells!='55'){
					
					mysqli_query($conexao2,"insert into cadastro set 
					categoria='".$_SESSION[$Script][CodCat]."',
					nome='".$nome."',
					email='".$email."',
					cod_pais= '55',
