...
$pergunta = "select * from cadastro where id='".$_GET[cod]."' ";
$resultado = mysqli_query($conexao2, $pergunta);
$d = mysqli_fetch_object($resultado);


$numeroDb = $d->cod_pais."".$d->cod_estado."".$d->telefone;
...
$celular = explode(' ',$_POST[celular]);	
	...
	//echo "<script>alert(' $cells e $_POST[validarNumero] ');</script>";
	if($cells != $_POST[validarNumero]){
		$tel_tipo = " tel_tipo='', ";
	}
...
$celular = explode(' ',$_POST[celular]);	
	...
	$cells = $celular[0].$celular[1].''.$celular[2];
		...
		$query = "insert into cadastro set 
...
<div class="card-body">
	 ...
	 <input type="hidden" name="destination" value="<?=$d->destination?>" />
	  ...
	  <input type="hidden" name="validarNumero" value="<?=$numeroDb?>" />	
