...
$Conf[script] = 'tabelas/ecommerce/email';
$Script = md5($Conf[script]);

$pergunta = "select * from email_planos order by ordem";
...
$valorPlano = ($d->valor_unitario) ; 
$porPlano = ($d->valor_unitario) / $d->quantidade ;	
...
?>	
	...
	<div class="col-lg-4 col-md-4 w3-card " >	
			...
			<span>R$<code style="color:#010096"><?=number_format($porPlano,5,",",".")?></code>/e-mail</span>	
		    </p>
		  ...
		  <div style="height:40px;" >
			<button type="button" class="btn btn-gradient-info btn-sm Comprar" Cod="<?=$d->id?>" ValorPlano="<?=number_format($valorPlano,2,",",".")?>"  ValorEmail="<?=number_format($d->valor_unitario,4,",",".")?>"   ValorPlanoSend="<?=$valorPlano?>" QuantidadePlano="<?=$d->quantidade?>">Comprar</button>
		  </div>
...
$(".Comprar").click(function(){
	...
	let ValorEmail = $(this).attr("ValorEmail");
	...
	let QuantidadePlano = $(this).attr("QuantidadePlano");
     ...
     SolicitandoCredir = $.confirm({
		...
		content: "url:./tabelas/ecommerce/info_vencimento_email.php?Cod="+Cod+"&ValorPlano="+ValorPlano+"&ValorEmail="+ValorEmail+"&ValorPlanoSend="+ValorPlanoSend+"&QuantidadePlano="+QuantidadePlano,
