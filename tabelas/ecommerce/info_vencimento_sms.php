<div id="AcessarMeusPlanosSms">
<?php
include('../../includes/connect.php');
include('../../includes/funcoes.php');
	
$Conf[script] = 'tabelas/ecommerce/info_vencimento_sms';
...
if(!$_SESSION[$Script][ValorSms]){$_SESSION[$Script][ValorSms] = $_GET[ValorSms];}
if($_GET[ValorSms]){$_SESSION[$Script][ValorSms] = $_GET[ValorSms];}
...
list($id_asaas) = mysqli_fetch_row(mysqli_query($conexao, "select id_asaas from usuarios where id = '".$_SESSION[id_usuario]."' "));	
...
$description = 'Credito SMS'; //Descrição da cobrança
...
function GerarFatura($conexao2,$api_token,$codPlano,$id_asaas,$billingType,$dueDate,$value,$description,$externalReference,$discountValue,$dueDateLimitDays,$fineValue,$interestValue){
	...
	$insert = " insert into asaas_cobranca_sms set
				object = '".$obj->object."',
				id = '".$obj->id."',
...
<label>Pagando R$ <b><?=$_SESSION[$Script][ValorWhas]?>/sms</b></label>
...
submitHandler: function( form ){
	...
	$.ajax({
		type: "POST",
		url: "./tabelas/ecommerce/info_vencimento_sms.php",
		data: dados,
		success: function( data )
		{
			$("#AcessarMeusPlanosSms").html(data);
...
function IrMeusPlanos(c,d){
	...
	$.ajax({
		type: "GET",
		url: "./tabelas/minhas_faturas/faturas_sms.php",
		data: {
			Cod:c,
			data_vencimento:d
		},
