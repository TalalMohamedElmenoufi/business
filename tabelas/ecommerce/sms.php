...
$Conf[script] = 'tabelas/ecommerce/sms';
$Script = md5($Conf[script]);

$pergunta = "select * from sms_planos order by ordem";
...
?>	
	...
	<div class="col-lg-4 col-md-4 w3-card " >	
			...
			<span>R$<code style="color:#010096"><?=number_format($d->valor_unitario,3,",",".")?></code>/sms</span>	
		    </p>
		  ...
		  <div style="height:40px;" >
			<button type="button" class="btn btn-gradient-info btn-sm Comprar" Cod="<?=$d->id?>" ValorPlano="<?=number_format($valorPlano,2,",",".")?>"  ValorSms="<?=number_format($d->valor_unitario,4,",",".")?>"   ValorPlanoSend="<?=$valorPlano?>" QuantidadePlano="<?=$d->quantidade?>">Comprar</button>
		  </div>
...
$(".Comprar").click(function(){
	...
	let ValorSms = $(this).attr("ValorSms");
	...
	let QuantidadePlano = $(this).attr("QuantidadePlano");
     ...
     SolicitandoCredir = $.confirm({
		...
		content: "url:./tabelas/ecommerce/info_vencimento_sms.php?Cod="+Cod+"&ValorPlano="+ValorPlano+"&ValorSms="+ValorSms+"&ValorPlanoSend="+ValorPlanoSend+"&QuantidadePlano="+QuantidadePlano,
