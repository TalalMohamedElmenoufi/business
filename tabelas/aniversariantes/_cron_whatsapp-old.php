...
while($d = mysqli_fetch_object($resultado)){
	...
	sendWhatsap($conexao,$d->id,$d->aniversariantes,$d->aniversariantes_whats,$timezone,$ip,$porta,$d->token);
	
}


function sendWhatsap($conexao,$cliente,$aniversariantes,$aniversariantes_whats,$timezone,$ip,$porta,$token){
...
$raniversariantes = mysqli_query($conexao2,$aniversariantes);	
	...
	//echo $MesAno ." - ". $dataHoje ." - ". $mensagem."<hr>";
	...
	if($creditoWhats > $QtEnviosWhats and $QtEnviosWhats > 0){
		...
		echo "CLI:<b style='color:red'>$cliente</b> ". $pergunta."<hr>";	
		...
		while($d = mysqli_fetch_object($resultado)){
			...
			if($mensagem){
				...
				//echo $d->id ." - ". $mensagem_enviar." - whats: $whatsapp <hr>"; 
				...
				mysqli_query($conexao2, " insert into t_".date('mY')."_aniversariantes set mensagem='".($mensagem_enviar)."', destination='".$whatsapp."', date = '".$data."', enviado='1', aniversario='enviado' ");
				$idSmg = mysqli_insert_id($conexao2);

				ValidarNumeros($conexao,$conexao2,$cliente,$timezone,$ip,$porta,$token,$d->id,$maxId,$mensagem_enviar,$whatsapp,$credito_atual_whats,$idSmg);					
		   }
...
function ValidarNumeros($conexao,$conexao2,$cliente,$timezone,$ip,$porta,$token,$id,$maxId,$mensagem_enviar,$whatsapp,$credito_atual_whats,$idSmg){
...
if($f){
		SendWhats($conexao,$conexao2,$cliente,$timezone,$ip,$porta,$token,$id,$maxId,$mensagem_enviar,$whatsapp,$credito_atual_whats,$idSmg);
}else{
	mysqli_query($conexao2,"update cadastro set tel_tipo = 'Invalido' where id = '".$id."' ");
	mysqli_query($conexao2, " update t_".date('mY')."_aniversariantes set aniversario='num_invalido' where id = '".$idSmg."' ");
}
...
function SendWhats($conexao,$conexao2,$cliente,$timezone,$ip,$porta,$token,$id,$maxId,$mensagemDb,$whatsapp,$credito_atual_whats,$idSmg){
	...
	if($result){
		...
		date_default_timezone_set($timezone);
		$data = date('Y-m-d H:i:s', time() ) ;		
		mysqli_query($conexao2, " update t_".date('mY')."_aniversariantes set aniversario='num_valido' where id = '".$idSmg."' ");


		if($id === $maxId){
			//echo "AT DB: $id e $maxId | $credito_atual_whats  <br>";
			mysqli_query($conexao, "update usuarios set creditos_msg='".$credito_atual_whats."' where id = '".$cliente."' ");
			AtualizarMensagem($conexao,$conexao2);
			//fecharConexoes($conexao,$conexao2);
		}				
