...
$pergunta = "select * from contatos_bot where id='".$_GET[cod]."' ";
...
$cel = explode(' ',$_POST[celular]);	
$celular = $cel[0]."".$cel[1]."".$cel[2];
	
		$query = "update contatos_bot set  
		nome='".($_POST[nome])."',
		email='".$_POST[email]."',
		whatsapp= '".$celular."',
...
$celular = $cel[0]."".$cel[1]."".$cel[2];	
		...
		$query = "insert into contatos_bot set 
		nome='".($_POST[nome])."',
		email='".$_POST[email]."',
		whatsapp= '".$celular."',
		situacao='".$_POST[situacao]."'
...
<div class="card-body">
        ...
        <form id="ValidarContatos" action="<?=$Conf[script]?>_form.php" method="post" target="pagina" enctype="multipart/form-data" >
			    ...
			    <div class="row">   
					...
					<?php 
					$pais = substr($d->whatsapp, 0, 2);
					$estado = substr($d->whatsapp, 2, 2);
			        $numero = substr($d->whatsapp, 4);
					$celular = $pais." ".$estado." ".$numero;
					?>
					<div class="col-lg-6" style="padding:1px">
						<span class="TituloForms">Celular</span> <span class="Sem9">Sem o número 9 da frente</span><br>
						  <i class="far fa-copyright Icons"></i>
						  <p><input InputForm id="celular" type="text" name="celular" class="form-control" placeholder="Celular"  value="<?=$celular?>" /></p>
					</div>
