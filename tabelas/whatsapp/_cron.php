...
function ListaUsuarios($conexao,$conexao2,$idGrupo,$timezone,$token,$status_whats_desc, $ip,$porta, $cliente){
	...
	while( $d3 = mysqli_fetch_object($resultado3) ){
		...
		elseif($status_whats_desc == "CONNECTED"){
			ValidarNumeros($conexao,$conexao2,$timezone,$cliente,$idGrupo,$d3->id,$d3->id_pesquisa,$d3->img,$d3->img_ext,$d3->pdf,$token,$mensagem,$celular,$ip,$porta);
		}				
	}
...
function ValidarNumeros($conexao,$conexao2,$timezone,$cliente,$idGrupo,$id,$id_pesquisa,$img,$img_ext,$pdf,$token,$mensagem,$whatsapp,$ip,$porta){
...
$msgReturn = $jsonObj->result;		
	

	
	if($msgReturn == "success" || $msgReturn == "invalido"){
		EnviarWhatsapp($conexao,$conexao2,$timezone,$cliente,$idGrupo,$id,$id_pesquisa,$img,$img_ext,$pdf,$token,$mensagem,$whatsapp,$ip,$porta);
...
$fields = array
(
	'img' => '/var/www/public_html/grupoelmenoufi/business/tabelas/whatsapp/upload/'.$img,
	'numbers' => $numeros,
	'mensagem' => '-'
);
...
$fields = array
(
	'pdf' => '/var/www/public_html/grupoelmenoufi/business/tabelas/whatsapp/upload/'.$pdf,
	'numbers' => $numeros,
	'mensagem' => '-'
...
$mensagemEnviar = $cfile." ".$link." ".$tituloDb." ".$descBoleDb;	
