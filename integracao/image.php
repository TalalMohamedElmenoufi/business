...
if($_POST){
	...
	$caminho_img = $dadosOut[caminho_img];
	$img = $dadosOut[img];
	$img_ext = $dadosOut[img_ext];	
}
...
$QtEnvios = count( explode(",",$numbers) );	
	...
	if( $dados['creditos_msg'] >= $QtEnvios ){
		Enviar($ip,$porta,$conexao,$dados[id],$dados[token],$mensagem,$numbers,$QtEnvios,$caminho_img,$img,$img_ext);
	}else{
		NaoEnviar();
	}
...
function Enviar($ip,$porta,$conexao,$codigo,$token,$mensagem,$celular,$QtEnvios,$caminho_img,$img,$img_ext){

$url = str_replace("-","/",$caminho_img);

$cfile = curl_file_create('/'.$url.'/'.$img,'image/'.$img_ext);
