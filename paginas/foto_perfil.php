...
$perguntaUser = "select id, nome, foto_perfil from usuarios where id = '".$_SESSION[id_usuario]."' ";
...
mkdir('../img/sistema/'.$pasta.'/', 0777, true);
mkdir('../img/sistema/'.$pasta.'/perfil/', 0777, true);
...
$foto_perfil = acentos($trat1) ;	
	...
	if($_FILES['foto_perfil']['name']){
		...
		$uploaddir = '../img/sistema/'.$pasta.'/perfil/';
		...
		if($result){
		 $dirPerfil = 'img/sistema/'.$pasta.'/perfil/'.$foto_perfil;
...
<form action="./paginas/foto_perfil.php" method="post" target="pagina" enctype="multipart/form-data" >
		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" s>
			...
			<div class="fileupload fileupload-new" data-provides="fileupload">
				<div class="fileupload-new thumbnail" style="left:0; width: 200px; height: 150px;">
					<img MinhaFoto src="<?=(($d->foto_perfil)?'img/sistema/'.$pasta.'/perfil/'.$d->foto_perfil:'assets/img/demoUpload.jpg')?>" alt="<?=$d->foto_perfil?>" />
				</div>
		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			$dir = $dir0."/img/sistema/".$pasta."/perfil/";
			  ...
			  if ($dh = opendir($dir)){
				while (($file = readdir($dh)) !== false){
				  ...
				  <div class="col-md-3 col-lg-3 imagemPerfil" style="text-align:center">
					<span FotoPerfil caminho="img/sistema/<?=$pasta?>/perfil/<?=$file?>" >
						<i class="fas fa-user-times ExcluirFperfil"></i>
					</span>  
					<img EditarPerfil caminho="img/sistema/<?=$pasta?>/perfil/<?=$file?>" NomeImg="<?=$file?>" class="FotoVez" src="img/sistema/<?=$pasta?>/perfil/<?=$file?>" alt="<?=$file?>" style="width:70%;" />
				  </div>
				  <?php
				  }
				}
	...
	$.confirm({
		...
		buttons: {
			Sim: {
				...
				action: function(){
					$.ajax({
						url: "./paginas/foto_perfil.php?ExcluirFoto="+caminho,
